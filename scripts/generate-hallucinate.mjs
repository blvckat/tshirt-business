...
const TY = 720         // baseline y for HALLUCINATE
const OFFSET = 8       // RGB channel offset px
const FONT_SIZE = 178
...
const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <!-- HALLUCINATE — RGB glitch layers -->

  <!-- Red layer: offset left -->
  <text
    x="${CX - OFFSET}" y="${TY}"
    font-family="BebasNeue" font-size="${FONT_SIZE}"
    fill="#ff2a2a" opacity="0.82"
    text-anchor="middle"
  >HALLUCINATE</text>
  ...
  <text
    x="${CX + OFFSET}" y="${TY}"
    font-family="BebasNeue" font-size="${FONT_SIZE}"
    fill="#00e5ff" opacity="0.82"
    text-anchor="middle"
  >HALLUCINATE</text>

  <!-- White main layer: center, on top -->
  <text
    x="${CX}" y="${TY}"
    font-family="BebasNeue" font-size="${FONT_SIZE}"
  ...
  <!-- Subtitle glitch — same RGB offset, small mono -->

  <!-- Red layer -->
  <text
    x="${CX - 3}" y="${TY + 68}"
    font-family="IBMPlexMono" font-size="24"
    fill="#ff2a2a" opacity="0.7"
    text-anchor="middle" letter-spacing="1.5"
  >// error: reality.exe not found</text>

  <!-- Cyan layer -->
  <text
    x="${CX + 3}" y="${TY + 68}"
    font-family="IBMPlexMono" font-size="24"
    fill="#00e5ff" opacity="0.7"
    text-anchor="middle" letter-spacing="1.5"
  >// error: reality.exe not found</text>
...
async function run() {
  console.log('Rendering HALLUCINATE...')
  const png = await sharp(Buffer.from(svg)).png().toBuffer()
  console.log(`PNG: ${(png.length / 1024).toFixed(0)}KB`)

  const fileName = `hallucinate-${Date.now()}.png`
  ...
  const dbRes = await fetch(`${SUPABASE_URL}/rest/v1/designs`, {
    ...
    body: JSON.stringify({ title: 'Hallucinate', image_url: publicUrl, status: 'pending' })
  })
